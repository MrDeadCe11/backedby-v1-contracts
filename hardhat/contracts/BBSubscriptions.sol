...
contract BBSubscriptions is IBBSubscriptions {
    ...
    event Renewed(uint256 subscriptionId);
    ...
    constructor(
        address bbProfiles,
        address bbTiers,
        address bbSubscriptionsFactory,
        address currency
    ) {
    ...
    */
    function performUpkeep(bytes calldata renewalData) external override {
        ...
        for (uint256 i; i < renewIndexes.length; i++) {
            ...
            if (
                _subscriptions[renewIndexes[i]].expiration < block.timestamp &&
                _subscriptions[renewIndexes[i]].cancelled == false
            ) {
                (uint256 tierSetId, uint256 contribution) = _bbSubscriptionsFactory.getSubscriptionProfile(
                    _subscriptions[renewIndexes[i]].profileId
                );

                // Check the subscription tier still exists, and the token is still accepted by the creator
                if (
                    _subscriptions[renewIndexes[i]].tierId <
                    _bbTiers.totalTiers(_subscriptions[renewIndexes[i]].profileId, tierSetId) &&
                    _bbTiers.isCurrencySupported(
                        _subscriptions[renewIndexes[i]].profileId,
                        tierSetId,
                        address(_currency)
                    )
                    ...
                    if (paid) {
                        ...
                        _subscriptions[renewIndexes[i]].expiration =
                            block.timestamp +
                            (DateTimeLibrary.getDaysInMonth(block.timestamp) * 1 days);
        ...
        uint256 refund = (gasAtStart - gasleft() + (56542 + (423 * renewCount))) *
            IBBGasOracle(_bbSubscriptionsFactory.getGasOracle()).getGasPrice();
    ...
    */
    function subscribe(
        ...
        require(
            msg.value >= _bbSubscriptionsFactory.getSubscriptionFee(address(_currency)),
            BBErrorCodesV01.INSUFFICIENT_PREPAID_GAS
        );

        if (_bbSubscriptionsFactory.isSubscriptionActive(profileId, tierId, msg.sender) == true) {
            (, , , bool cancelled) = IBBSubscriptions(
                _bbSubscriptionsFactory.getDeployedSubscriptions(
                    _bbSubscriptionsFactory.getSubscriptionCurrency(profileId, tierId, msg.sender)
                )
            ).getSubscriptionFromProfile(profileId, tierId, msg.sender);
    ...
    */
    function checkUpkeep(bytes calldata checkData) external view override returns (bool, bytes memory) {
        (uint256 lowerBound, uint256 upperBound, uint256 minRenews, uint256 maxRenews, address refundReceiver) = abi
            .decode(checkData, (uint256, uint256, uint256, uint256, address));
        ...
        for (uint256 i; i < checkLength; i++) {
            ...
            if (
                _subscriptions[subscriptionIndex].expiration < block.timestamp &&
    ...
    */
        ...
        )
    {
        ...
        return (id, _subscriptions[id].price, _subscriptions[id].expiration, _subscriptions[id].cancelled);
    }
